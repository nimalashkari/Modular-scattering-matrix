...
\section{Characteristic Functions in Conformal Perturbation Theory}\label{section:conformalperturbation}
...
\begin{align}
    \int_0^1 dw \; h_{reg}(u,X) =&\; \pi(1-X^2)\int_0^1 \, d\tau \int_0^1 dw \;\frac{\left(1 - (1-X^2)\sin^2(\pi \tau)\right)^{\Delta-2} -1}{\sinh^2(\pi (u+sw+i\tau))}\\
    =&\; (1-X^2)\int_0^1 \, d\tau \; \left[\left(1 - (1-X^2)\sin^2(\pi \tau)\right)^{\Delta-2} -1\right] \nonumber\\
    &\times\left( \coth(\pi(u+i\tau)) - \coth(\pi(u+s+i\tau)) \right)
\end{align}
We can expand $\Omega^{\Delta-2}(-i\tau,X)$ as a cosine series in $\tau$
\begin{align}
    \Omega^{\Delta-2}(-i\tau,X) - 1 = \sum_{n=0}^\infty a_n \cos(2\pi n \tau).
\end{align}
...
\begin{align}
    ...
    =&\; - \epsilon \ln \left(\frac{1+X^2}{2}(1+\frac{1-X^2}{1+X^2}\cos(2\pi \tau))\right) + O(\epsilon^2). %\\
    %= &\; \epsilon \sum_{n=0}^{\infty} \left( \right)\cos(2 \pi n \tau) + O(\epsilon^2) 
\end{align}
We use the identity ($|r|<1$)
\begin{align}
    \ln(1+r^2 + 2r\cos(\theta)) =&\; \ln(1+re^{i\theta}) + \ln (1+re^{-i\theta}),\\
    =&\; \sum_{n= 1}^{\infty} (-1)^{n+1}\frac{r^n e^{in\theta}}{n} +\sum_{n= 1}^{\infty} (-1)^{n+1}\frac{r^n e^{-in\theta}}{n}, \\
    =&\; \sum_{n= 1}^{\infty}(-1)^{n+1} \frac{2r^n \cos(n\theta)}{n} .
\end{align}
By putting $r = \frac{(1-|X|)^2}{1-X^2}$, we have
\begin{align}
    \Omega^{\Delta-2}(-i\tau,X) - 1 =&\; \epsilon \left(\sum_{n=1}^{\infty} \left( \right)\cos(2 \pi n \tau) \right) + O(\epsilon^2)
\end{align}
