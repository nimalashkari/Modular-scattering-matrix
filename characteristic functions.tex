...
We compute $f(z)= \;_\lambda\bra{O(X)}\Delta_{A,\lambda}^{1/2+iz}\Delta_{B,\lambda}^{-iz}\ket{O(Y)}_\lambda$ in a QFT parametrized by $\lambda$. When both $A$ and $B$ are wedge, the same logic follows the CFT case, as the modular flows are still boost. We consider the inclusion $B\subset A$ where $B$ is a unit double cone centered at the origin and $A$ is the wedge with the end point $(0,-x_w)$, $x_w >0$. We have a similar setup where $Y\in B$, $X\in A$. We have
\begin{align}
    f(z) =&\;_\lambda\bra{O(X)}\Delta_{A,\lambda}^{1/2+iz}\Delta_{B,\lambda}^{-iz}\ket{O(Y)}_\lambda \\
    =&\;\bra{\Omega_\lambda}O(X)^\dagger \Delta_{A,\lambda}^{1/2+iz}\Delta_{B,\lambda}^{-iz}O(Y) \ket{\Omega_\lambda}\\
    =&\;\bra{\Omega_\lambda}\left(\Delta_{A,\lambda}^{1/2} O(X)^\dagger \Delta_{A,\lambda}^{1/2}\right)\left( \Delta_{A,\lambda}^{iz} \Delta_{B,\lambda}^{-iz}O(Y) \Delta_{B,\lambda}^{+iz}\Delta_{A,\lambda}^{-iz}\right)\ket{\Omega_\lambda}\\
    =& \; \bra{\Omega_\lambda}\left(\Delta_{A,\lambda}^{1/2} O(X)^\dagger \Delta_{A,\lambda}^{1/2}\right)\left( \Delta_{A,\lambda}^{iz} \Delta_{B,\lambda}^{-iz}O(Y) \Delta_{B,\lambda}^{+iz}\Delta_{A,\lambda}^{-iz}\right)\ket{\Omega_\lambda}\\
    =&\; _0\bra{O(X)}\Delta_{A,0}^{1/2+iz}\Delta_{B,0}^{-iz}\ket{O(Y)}_0 + \lambda f^{(1)}(z) + O(\lambda^2),
\end{align}
where
\begin{align}
    f^{(1)}(z) =&\; f^{(1)}_{\text{flow}}(z) + f^{(1)}_{\text{vecL}}(z) + f^{(1)}_{\text{vecR}}(z),\\
    f^{(1)}_{\text{flow}}(z) \equiv&\; \;_0\bra{O(X)}\left(\Delta_{A,\lambda}^{1/2+iz}\Delta_{B,0}^{-iz}\right) \left(...\right)\ket{O(Y)}_0, \\
    f^{(1)}_{\text{vecL}}(z) \equiv&\; ,\\
    f^{(1)}_{\text{vecR}}(z) \equiv&\; ,
\end{align}
...
\end{align}
\KL{(A factor of 4 missing? Check later) }where 
\begin{align}
...
\begin{align}
    Q(u,X) \equiv&\;\int_0^1 dw \; h_{reg}(u+sw,X) \nonumber\\
...
\begin{align}
    Q(u,X) =&\; Q^{(1)}(u,X) + O(\epsilon^2)\\
    Q^{(1)}(u,X) =&\;2(1-X^2)\epsilon\left\{ \text{sign}(u)\ln\left[1+|X| +(1-|X|)e^{-2\pi|u|}\right] \right. \nonumber\\
    & \left.- \text{sign}(u+s)\ln\left[1+|X| +(1-|X|)e^{-2\pi|u+s|}\right] \right\}.
\end{align}
Thus, the cocycle is 
\begin{align}
    u_{s,\lambda}=&\; 1 - i4\pi s\lambda\int_0^1 dw  \int_{-1}^1 dX \, (1-X^2)e^{-iswK_0} O(X) e^{iswK_0} \nonumber\\
    &+ i2\pi s\lambda\int_0^1 dw  \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, h_{reg}(u+sw,X) \Omega^{-\Delta}(u,X)O(t_u,x_u)\\
    =&\; 1 + i2\pi s\lambda \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, M(u,X)O(t_u,x_u) +O(\lambda^2),
\end{align}
where 
\begin{align}
    M(u,X) =&\; \left[Q(u,X) + 2(1-X^2)\frac{1}{s} \textbf{1}_{(-s,0)}(u)\right]\Omega^{-\Delta}(u,X)
\end{align}
