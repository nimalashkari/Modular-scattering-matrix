...
\begin{gather}
    u_{s,\lambda} = e^{-isK_0}e^{isK_\lambda} = 1 + i\pi s\lambda \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, Q(u,X)O(t_u,x_u) +O(\lambda^2),\\
    \Omega_\lambda = \Omega_0 + \lambda\Omega^{(1)} + O(\lambda^2).
\end{gather}
Therefore, the three correction terms are
\begin{align}
    f^{(1)}(z) =&\; f^{(1)}_{\text{flow}}(z) + f^{(1)}_{\text{vecL}}(z) + f^{(1)}_{\text{vecR}}(z),\\
    f^{(1)}_{\text{flow}}(z) \equiv&\; \;_0\bra{O(X)}\left(\Delta_{A,0}^{1/2+iz}\Delta_{B,0}^{-iz}\right) \left(...\right)\ket{O(Y)}_0, \\
    f^{(1)}_{\text{vecL}}(z) \equiv&\; \bra{\Omega^{(1)}} ... \ket{\Omega_0},\\
    f^{(1)}_{\text{vecR}}(z) \equiv&\; \bra{\Omega_0} ... \ket{\Omega^{(1)}}.
\end{align}
...
\begin{align}
    ...
    =& \pi \lambda\int_I dX \int_{-\infty}^{\infty} du \, h(u,X) e^{iuK_0} O(X) e^{-iuK_0},
\end{align}
where 
\begin{align}
    h(u,X) = -\int_0^1\, d\tau \frac{J(\tau,X)\Omega^{\Delta}(-i\tau,X)}{2\sinh^2(\pi (u+i\tau))} 
\end{align}
...
\begin{gather}
    ...
    h(u,X) = -\pi(1-X^2)\int_0^1\, d\tau \frac{\Omega^{\Delta-2}(-i\tau,X)}{2\sinh^2(\pi (u+i\tau))}
\end{gather}
...
\begin{gather}
    h(u,X) =  (1-X^2)\delta(u) + h_{reg}(u,X), \\
    h_{reg}(u,X)= -\pi(1-X^2)\int_0^1\, d\tau \frac{ (\Omega^{\Delta-2}(-i\tau,X) -1)}{2\sinh^2(\pi (u+i\tau))} .
\end{gather}
...
\begin{align}
    ...
    =&\; 1 + i\pi s\lambda\int_0^1 dw  \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, h(u+sw,X) e^{iuK_0} O(X) e^{-iuK_0} \\
    =&\; 1 + i\pi s\lambda\int_0^1 dw  \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, h(u+sw,X) e^{iuK_0} O(X) e^{-iuK_0}\\
    =&\; 1 + i\pi s\lambda\int_0^1 dw  \int_{-1}^1 dX \, (1-X^2)e^{-iswK_0} O(X) e^{iswK_0} \nonumber\\
...
\begin{align}
    Q_{\text{nonloc}}(u,X) \equiv&\;\int_0^1 dw \; h_{reg}(u+sw,X) \nonumber\\
    =&\; -\pi(1-X^2)\int_0^1 \, d\tau \int_0^1 dw \;\frac{\left(1 - (1-X^2)\sin^2(\pi \tau)\right)^{\Delta-2} -1}{2\sinh^2(\pi (u+sw+i\tau))}\\
    =&\; -\frac{(1-X^2)}{2}\int_0^1 \, d\tau \; \left[\left(1 - (1-X^2)\sin^2(\pi \tau)\right)^{\Delta-2} -1\right] \nonumber\\
...
\begin{align}
    Q_{\text{nonloc}}(u,X) =&\; Q^{(1)}_{\text{nonloc}}(u,X) + O(\epsilon^2)\\
    Q^{(1)}_{\text{nonloc}}(u,X) =&\; -(1-X^2)\epsilon\left\{ \text{sign}(u)\ln\left[1+|X| +(1-|X|)e^{-2\pi|u|}\right] \right. \nonumber\\
...
\begin{align}
    ...
    &+ i\pi s\lambda\int_0^1 dw  \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, h_{reg}(u+sw,X) \Omega^{-\Delta}(u,X)O(t_u,x_u)\\
    =&\; 1 + i\pi s\lambda \int_{-1}^1 dX \int_{-\infty}^{\infty} du \, Q(u,X)O(t_u,x_u) +O(\lambda^2),
\end{align}
where 
\begin{align}
    Q(u,X) =&\; \left[Q_{\text{nonloc}}(u,X) + Q_{\text{loc}}(u,X) \right]\Omega^{-\Delta}(u,X), \\
    Q_{\text{loc}}(u,X) \equiv &\; (1-X^2)\frac{1}{s} \textbf{1}_{(-s,0)}(u).
\end{align}

In the case where the perturbed QFT is another CFT ($\Delta = 2$), the non-local term $Q_{\text{nonloc}}$ vanishes. The local term $Q_\text{loc}$ combined with the original modular Hamiltonian gives the same geometric flow in the new CFT. 
