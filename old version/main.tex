...
In scattering theory, we start with an interacting Hamiltonian that split as 
\begin{eqnarray}
H=H_0+V
\end{eqnarray}
...
\end{eqnarray}
with the property that coarse-grained multi-particle states $\ket{\Phi_\alpha^\pm}$ behaves like free multi-particle eigenstates $\ket{\Phi_\alpha}$. The make this coarse-graining rigorous, we consider states of the interacting theory that belong to a microcanonical ensemble of small width $\Delta E$ with a smooth wave-packet $g(\alpha)$ and evolve them for long times $t\gg 1/(\Delta E)$. Then, the requirement is that 
\begin{eqnarray}
\|\int e^{-i E_\alpha t}g_\alpha (\ket{\Psi^\pm_\alpha}-\ket{\Phi_\alpha})\|
\end{eqnarray}
can be made arbitrarily small for large $t$. The intuition is that as while the free multi-particle wave-packet $\ket{\Phi_\alpha}$ can never be an eigenkets of the interacting theory, as we separate excitations by a large amount, the interaction fall off so fast, that they can be approximated arbitrarily well by suitably coarse-grained eigenkets $\ket{\Psi_\alpha^\pm}$. The asymptotic states can be defined as 
\begin{eqnarray}
...
\begin{eqnarray}
\braket{\Phi_\beta|S \Phi_\beta}&&=\braket{\Psi_\alpha^+|\Psi_\beta^-}=\lim_{t\to \infty}\braket{\Phi_\alpha|\Omega(t)^\dagger \Omega(-t)|\Phi_\beta}\ .
\end{eqnarray}
