...
% Title
\title{Subalgebra Flows in Ergodic Theory and QFT}
...
% \section{Subalgebra flows in classical ergodic theory}
...
% Subregion and subsystem coarse-grainings organize incomplete information differently. A subregion coarse-graining resolves microstates within a selected region and identifies those outside it, whereas a subsystem coarse-graining identifies microstates that share the same observed variables. Evolution transports the resolved region in the first case and the fibers of indistinguishable microstates in the second. Their invariance has correspondingly different meanings: an invariant region defines a conserved event, while an invariant subsystem defines an autonomous factor dynamics. Ergodicity excludes nontrivial conserved events but permits nontrivial autonomous factors.
...
% \subsection{Subalgebra Hamiltonian flows}
...
The passage from a reversible microscopic description to an apparently irreversible macroscopic one is often attributed to information loss during coarse-graining. As we review below, this information loss can be quantified by associating an entropy to a coarse-graining. A relative phase constructed from the fine-grained and coarse-grained densities generates this entropy and provides a first classical avatar of a coarse-graining flow. This phase acts on Hilbert space vectors by multiplication, with a value that depends on the microstate and the flow parameter. Its conjugation action on the commutative observable algebra is trivial. To obtain a state-dependent flow on phase space, we will later introduce a Poisson bracket.
...
\paragraph{Dynamics:} Dynamics will be described by a measurable one-parameter group of invertible, measure-preserving transformations $\Phi_t:X\to X$. General dynamics need not be smooth. In physics, we are often interested in smooth flows described by a one-parameter family of volume-preserving diffeomorphisms. Taking a $t$-derivative of how a point $x\in X$ moves under this flow defines a vector field $V$, which preserves the volume measure if and only if $\mathcal{L}_V\mu=0$; i.e. it is divergence-free with respect to $\mu$. 
...
\[
  U_t^{\rho_0}:=\Mop_{\rho_0^{it}}.
\]
The parameter $t$ is an auxiliary modular or phase parameter. Nothing in this construction identifies it with laboratory time. In particular, $U_t^\rho$ does not transport points of $X$; it only rotates the phase of a Hilbert space vector pointwise\footnote{For faithful normal states on the same abelian algebra, $u_t=(\rho/\rho_0)^{it}$ is the Connes Radon-Nikodym cocycle. Relative modular operators remain nontrivial, even though the modular automorphism group of each state is trivial.}.
...
\subsection{Ergodic hierarchy and subalgebra flows}
...
\paragraph{Ergodicity and conserved events:}
...
\begin{align}
    ...
    =\mathbb C1\ .
\end{align}
An invariant event $O$ gives a conserved projection $\chi_O$. Thus ergodicity excludes a decomposition into two invariant subregions of positive measure. It does not exclude an invariant subsystem $\mB\subset\mA$: the condition $\alpha_t(\mB)=\mB$ permits observables in $\mB$ to evolve into one another. Such a subsystem is an autonomous factor of the dynamics, and its own dynamics is ergodic whenever the full system is ergodic.

\paragraph{Correlation functions and mixing:}
For $f,g\in\mA$, define the connected correlation function
\begin{align}
    C_{f,g}(t)
    :=\mu\bigl(f^*\alpha_t(g)\bigr)-\mu(f^*)\mu(g),
    \qquad \mu(f)=\int_X f\,d\mu\ .
\end{align}
For indicator functions, this becomes
\begin{align}
    C_{\chi_O,\chi_V}(t)
    =\mu\bigl(O\cap\Phi_t(V)\bigr)-\mu(O)\mu(V)\ .
\end{align}
It measures the statistical dependence between an event and a transported event. Ergodicity is equivalent to the vanishing of the time-averaged connected correlation for every pair of observables:
\begin{align}
...
\paragraph{One-sided subalgebra flows and the Kolmogorov property:}
...
Each $\mB_t$ has a $\mu$-preserving conditional expectation $E_t:\mA\to\mB_t$. For $s\geq t$, the tower property gives
\begin{align}
    E_sE_t=E_s\ .
\end{align}
For a fixed state with density $\rho$, its coarse-grained densities $\rho_t=E_t(\rho)$ therefore satisfy
\begin{align}
    H(\rho_s)\geq H(\rho_t),
    \qquad
...
\paragraph{Poisson subsystems:}
Let $\pi:X\to Y$ be a smooth surjective Poisson submersion:
\begin{align}\label{eq:classical-Poisson-subsystem}
    \{\pi^*f,\pi^*g\}_X=\pi^*\{f,g\}_Y,
    \qquad f,g\in C^\infty(Y).
\end{align}
The smooth coarse observables $\mathcal B=\pi^*C^\infty(Y)$ are closed under the bracket. This condition allows us to speak of Hamiltonian dynamics within the coarse description. It does not imply that an arbitrary Hamiltonian on $X$ preserves $\mathcal B$.
...
\begin{align}\label{eq:classical-marginal-Hamiltonian}
    ...
    K_Y=-\log\rho_Y.
\end{align}
The expectation $E_Y$ from Section~\ref{subsec:classical-region-subsystem} extends this marginal back to $X$ as $\rho_0(y,z)=\rho_Y(y)$. Its Hamiltonian is $K_0=\pi^*K_Y$, whose flow is
\begin{align}
...
\paragraph{When does the reduced dynamics agree with the full dynamics?}
...
\end{align}
Normalization of the marginal fixes $\int_Z\rho_Z\,d\mu_Z=1$. In fact, autonomy under any reduced smooth dynamics already forces this factorization: independence of the observed Hamiltonian velocity from $z$ makes $d_YK_\rho$ independent of $z$, and integration on connected $Y$ gives an additive Hamiltonian. This conclusion uses the product symplectic structure and the special choice $K_\rho=-\log\rho$; it is stronger than the condition for an autonomous subsystem under general measure-preserving dynamics.
...
\paragraph{Subregions and the passage to QFT:}
