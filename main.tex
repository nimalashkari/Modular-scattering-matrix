...
Finish a draft on ``Characteristic functions in QFT"

\NL{Slogan or title for the longer paper: Modular S-matrix is local approximation of global Hamiltonian}

\KL{ball in wedge $\epsilon\to0$ limit, tower, no commutant of translation, CFT perturbation }
...
\subsubsection{Wedge insdie Wedge}
...
\subsubsection{Ball inside Wedge}
...
We apply the conformal perturbation method to compute the modular flow of a ball-shaped region. We follow the method in \cite{} and consider the lagrangian with perturbation
\begin{align}
    \mathcal{L}_\text{CFT} \to \mathcal{L}_\text{CFT} + \lambda O(x),
\end{align}
where $O(x)$ is a conformal primary and $\lambda$ is a small parameter. The modular flow $e^{isK_\lambda}$ of an algebra $\mathcal{A}$ can be computed perturbatively
\begin{gather}
    ...
    \rho^{-1}\delta\rho = \lambda\int_I dX\int_0^1 d\tau 2\pi\Omega^{\Delta - D}(\tau,X) e^{\tau K_0} O(X) e^{-\tau K_0} ,
\end{gather}
where $\Omega$ is a conformal factor that depends on the geometric action of the flow $e^{is K_0}$, $\Delta$ is the conformal dimension of $O$, and $D$ is the spacetime dimension. Notice that the second line is valid only for operators in the algebra. By substituting $\rho^{-1}\delta_\lambda\rho$ into the expression of $\delta_\lambda K_\lambda$ and changing the order of integration, we can choose $\theta = \tau$ to cancel the Euclidean part. The expression is purely Lorentzian as pointed out by \cite{}
\begin{align}
    \delta_\lambda K_\lambda=& \lambda\int_I dX\int_0^1 d\tau \int_{-\infty}^{\infty} du \, \frac{\pi^2}{\sinh^2(\pi (u+i\tau))}   \Omega^{\Delta - D}(\tau,X) e^{iuK_0} O(X) e^{-iuK_0} \\
    =& \pi^2 \lambda\int_I dX \int_{-\infty}^{\infty} du \, h(u,X) e^{iuK_0} O(X) e^{-iuK_0},
\end{align}
where 
\begin{align}
    h(u,X) = \int_0^1\, \frac{ d\tau }{\sinh^2(\pi (u+i\tau))} \Omega^{\Delta - D}(\tau,X).
\end{align}
For simplicity, we consider $D = 1+1$, $I$ to be the interval $(-1,1)$ at $t = 0$, and $B$ the double cone of $I$. We can generalize the result to a higher dimension or to a different interval/ball-shaped region. We have 
\begin{gather}
    e^{isK_0} O(t,x) e^{-isK_0} = \tilde{\Omega}^{-\Delta} O(t_s,x_s), \\
    t_s = \tilde{\Omega}^{-1}\left[t \cosh(2\pi s) + \frac{(1+t^2-x^2)}{2}\sinh(2\pi s) \right],\\
    x_s = \tilde{\Omega}^{-1}x, \\
    \tilde{\Omega}^{-1} = \frac{2}{1-t^2+x^2+(1+t^2-x^2)\cosh(2\pi s) + 2t \sinh(2\pi s)}.
\end{gather}
For $O(t=0,x)$, we have
\begin{gather}
    t_s = \tilde{\Omega}^{-1}\left[\frac{(1-x^2)}{2}\sinh(2\pi s) \right],\\
    x_s = \tilde{\Omega}^{-1}x, \\
    \tilde{\Omega}^{-1} = \frac{2}{1+x^2+(1-x^2)\cosh(2\pi s)}.
\end{gather}
The conformal factor $\Omega$ is
\begin{align}
    \Omega(\tau,x) = \tilde{\Omega}|_{s=-i\tau} = \frac{1+x^2+(1-x^2)\cos(2\pi \tau)}{2}
\end{align}
...
\begin{align}
    ...
    =& \, \phi(t,x) + i\pi^2 \lambda s \int_{-\infty}^{\infty} du \int_{-1}^1 dX \int_0^1 dw\, h(u+sw,X)\left[  e^{iuK_0} O(t=0,x=X) e^{-iuK_0} ,\phi(t,x)\right],\\
...
\begin{align}
    ...
    &\times \braket{\Omega_\lambda|O(X_1) O(t_u,x_u) O(X_2)|\Omega_\lambda}
\end{align}
